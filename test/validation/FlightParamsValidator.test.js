...
describe('FlightParamsValidator', () => {
  ...
  it('validates list fields', () => {
    expectValidationError(
      () => FlightParamsValidator.validate({ flights: 'INVALID' }),
      /'flights' contains invalid IATA flight number/
    );
    expectValidationError(
      () => FlightParamsValidator.validate({ callsigns: ['LONGCALLSIGN'] }),
  ...
  it('validates airline specific filters', () => {
    ...
    expectValidationError(
      () => FlightParamsValidator.validate({ painted_as: ['A1'] }),
      /'painted_as' contains invalid airline ICAO code/
    );
  });

  it('validates airport and route filters', () => {
    expectValidationError(
      () => FlightParamsValidator.validate({ airports: '123' }),
      /'airports' contains invalid airport/
    );
    expectValidationError(
      () => FlightParamsValidator.validate({ routes: 'KJFK-LAX-SEA' }),
      /'routes' contains invalid route/
    );
    expectValidationError(
      () => FlightParamsValidator.validate({ aircraft: 'A320' }),
      /'aircraft' contains invalid aircraft/
    );
  });

  it('validates altitude ranges', () => {
    expectValidationError(
      () => FlightParamsValidator.validate({ altitude_ranges: '10000-0' }),
  ...
  it('validates squawks, categories, data sources, and airspaces', () => {
    expectValidationError(
      () => FlightParamsValidator.validate({ squawks: '8888' }),
      /'squawks' contains invalid squawk code/
    );
    expectValidationError(
      () => FlightParamsValidator.validate({ categories: 'Z' }),
      /'categories' contains invalid category/
    );
    expectValidationError(
      () => FlightParamsValidator.validate({ data_sources: 'RADAR' }),
      /'data sources' contains invalid data source/
    );
    expectValidationError(
      () => FlightParamsValidator.validate({ airspaces: [] }),
      /'airspaces' must contain at least one range/
    );
  });

  it('validates ground speed and limit ranges', () => {
    expectValidationError(() => FlightParamsValidator.validate({ gspeed: 6000 }), /'gspeed' must be an integer/);
    expectValidationError(() => FlightParamsValidator.validate({ limit: 40000 }), /'limit' must be an integer/);
